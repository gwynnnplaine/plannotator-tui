...
impl App {
    ...
    fn page_by(&mut self, delta: i64) {
        ...
        // The block that owns the target row, or the next one when that block starts above
        // the view (a tall block, or a gap row).
        let owner = blocks.partition_point(|b| b.first_row <= target);
        let block = match owner.checked_sub(1) {
            Some(i) if blocks.get(i).is_some_and(|b| view.contains(&b.first_row)) => i,
            _ => owner,
        };
        if blocks.get(block).is_some_and(|b| view.contains(&b.first_row)) {
